...
func _physics_process(_delta: float) -> void:
	...
	for i in steps:
		...
		if hit.is_empty(): # HACK: I'M PATCHING THIS FUNCTION TO SUPPORT TERRAIN3D -fragskye
			var height: float = Global.environment.terrain_3d.data.get_height(q.from)
			if height != NAN:
				hit = {
					"position": Vector3(q.from.x, height, q.from.z),
					"normal": Global.environment.terrain_3d.data.get_normal(q.from)
				}
